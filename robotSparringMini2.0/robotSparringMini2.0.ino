...
const bool reverseSide = false;

const int POS_START         = 0;
const int POS_END           = 180;
const int POS_MIDDLE        = 90;
const int POS_START_PLUS    = 30;
const int POS_END_MINUS     = 150;
const int POS_MIDDLE_PLUS   = 120;
const int POS_MIDDLE_MINUS  = 60;

enum PunchStraightType {
  STRAIGHT,
  STRAIGHT_X2,
  STRAIGHT_X3,
  FEINT
...
function getDelay(int posStart, int posEnd) {
  const int DEGREE = 60;
  const int SPEED  = 140;
  const float angle = abs(posEnd - posStart);
  return ceil(angle * SPEED / DEGREE);
}

function servoMove(int targetPosition) {
  ...
  servo.write(servoPulse);
  delay(servoDelay);
}


void executeStraightAction(PunchHookType punchType, int difficulty) {
  Serial.printf("PunchType: %d, Difficulty: %d\n", punchType, difficulty);
  switch (punchType) {
    case STRAIGHT:
...
void executeHookAction(PunchHookType punchType, int difficulty) {
  Serial.printf("PunchType: %d, Difficulty: %d\n", punchType, difficulty);
  switch (punchType) {
    case HALF_HOOK:
